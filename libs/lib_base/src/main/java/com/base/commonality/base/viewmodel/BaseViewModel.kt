...
import com.base.commonality.base.bean.ReqState
import com.base.commonality.ktx.log
import com.base.commonality.utils.NetworkUtil
...
abstract class BaseViewModel : ViewModel() {
    ...
    fun <T : Any> requestDataWithSingleFlow(
        ...
        viewModelScope.launch {
            ...
            try {
                baseData = request()
                log("baseData  baseData  state  ${baseData.state}")
                when (baseData.state) {
                    ReqState.Success -> {
                        log("baseData  baseData  data   ${baseData.data}")
                        _normalChannel.send(true)
                ...
                log("baseData  baseData  showLoading  ${showLoading}")
                if (showLoading) {
                    loadFinish()
                }
            }
