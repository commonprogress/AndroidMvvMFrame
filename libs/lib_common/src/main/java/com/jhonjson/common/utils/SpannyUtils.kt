package com.jhonjson.common.utils
...
class Spanny : SpannableStringBuilder {
    private var flag = SPAN_EXCLUSIVE_EXCLUSIVE
    ...
    fun appendText(text: CharSequence): Spanny {
        append(text)
        return this
    }
    ...
    companion object {
        ...
        fun spanText(text: CharSequence, vararg spans: Any?): SpannableString {
            ...
            for (span in spans) {
                spannableString.setSpan(span, 0, text.length, SPAN_EXCLUSIVE_EXCLUSIVE)
            }
            return spannableString
        }
